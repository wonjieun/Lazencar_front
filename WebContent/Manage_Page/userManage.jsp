...
<script type="text/javascript">		$(document).ready(function(){		$("#searchMember").click(function(){			var constraint;		//카테고리에 따른 조건 지정 변수						var category = $("#category").val();			var content = $("#search_content").val();			var sort = $("#sort").val();									if(category=="회원 계정"){				constraint = mem_id;		//dto에서 mem_id를 꺼내옴.			} else if(category=="출생 년도"){				constraint = "19"+mem_jumin;		//19+주민번호 앞 2글자 빼ㅇ기			} else if(category=="면허 여부"){				constraint = mem_license			} else{// 				showAll			}									//입력 데이터 확인 코드			console.log("카테고리 선택 : "+category);			console.log("검색내용 : "+content);			console.log("정렬 : "+sort);								});		//각 입력칸들의 데이터를 받아와서// 		console.log(); 받아와지는지확인하고		$.ajax({			type : "POST",			url : "/userManage.do",			data : {				memId:mem_id,				memPw:mem_pw,				memName:mem_name,				memJumin:mem_jumin,				memPhone:mem_phone,				memAddr:mem_addr,				memEmail:mem_email,				memLicense:mem_license				},			dataType : "json",			success : function(data) {				alert("회원목록을 불러옵니다.");						},			error : function(e) {				console.log("------error------");				console.log(e.responseText);			}			});					}); </script>
</head>
<body>
	<div id="menu">
		<ul class="level1">
