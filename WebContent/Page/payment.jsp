...
// 		});
	...
	function requestPayment() {
		var title = "${title}"; //주문명을 결제번호로 생성
		var amount = ${amount}; //금액
		var buyer_email = "${email}";//사용자 이메일
		var buyer_name = "${name}"; //사용자 이름
		var buyer_tel = "${phone}"; //사용자 폰번호
		...
		IMP.request_pay({
			...
			if (rsp.success) { // 결제 성공 로직
				...
				var $form = $("<form>").attr("action","/reservation/payment.do").attr("method","post");
				$("<input>").attr("type","hidden").attr("name","result").attr("value","결제 완료").appendTo($form);
...
// 						alert(msg);
				...
				var $form = $("<form>").attr("action","/reservation/payment.do").attr("method","post");
				$("<input>").attr("type","hidden").attr("name","result").attr("value","결제 실패").appendTo($form);
				$form.appendTo($(document.body));
				$form.submit();
